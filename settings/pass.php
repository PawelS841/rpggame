...
<aside id="menu">
    ...
    <main id="glowny">
        <form action="pass_change.php" method="post">
            <input type="password" placeholder=" WPISZ STARE HAŁSO" name="old_pass">
            <input type="password" placeholder=" WPISZ NOWE HAŁSO" name="new_pass">
            <input type="password" placeholder=" POWRÓRZ NOWE HAŁSO" name="repeat_pass">
            <?php
            if(isset($_SESSION['pass_change_error']))
            {
                if($_SESSION['pass_change_error']==1)
                    echo "<p>BŁEDNE HASŁO</p>";
                if($_SESSION['pass_change_error']==2)
                    echo "<p>HASŁO NIE SĄ TAKIE SAME</p>";
                if($_SESSION['pass_change_error']==3)
                    echo "<p>HASŁO JEST ZA SŁABE</p>";
                
                unset($_SESSION['pass_change_error']);
            }
            ?>
            <input type="submit">
        </form>
