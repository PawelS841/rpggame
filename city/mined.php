...
if(isset($_POST["id"]))
{
    ...
    $lvl=$row['SMITHING'];
    $sql="SELECT mine.name as name,image,mine.id as id,speed,lvl,drop_chance,mine.Item_ID as item,price FROM mine JOIN items ON items.ID=mine.Item_ID WHERE mine.id={$id}";
    $resoult=mysqli_query($connect,$sql);
    $row=mysqli_fetch_assoc($resoult);
