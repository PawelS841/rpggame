...
<aside id="menu">
    ...
    <main id="glowny">
        ...
        <?php
            ...
            $lvl=$row['FISHING'];
            $sql="SELECT fish.name as name,image,fish.id as id FROM fish JOIN items ON items.ID=fish.Item_ID WHERE lvl<={$lvl}";
            $resoult=mysqli_query($connect,$sql);
            while($row=mysqli_fetch_assoc($resoult))
            {
                echo"
                <section class='source' onclick='start_fishing({$row['id']})'>
